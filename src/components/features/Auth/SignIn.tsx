...
const SignIn: Component = () => {
  ...
  // Set once the password step passes for an account with two-factor auth.
  const [mfaChallenge, setMfaChallenge] = createSignal<{ token: string; email: string } | null>(
    null,
  );
  ...
  const handleSubmit = async (e: Event) => {
    ...
    try {
      const outcome = await login(data.email, data.password, data.rememberMe || false);

      // Password was right, but the account has two-factor auth. No session
      // exists yet — swap the form for the code step and keep the challenge
      // token, which is the only thing that can finish this login.
  ...
  const MfaStep = () => (
    <AuthLayout>
      <div class="text-left mb-6 space-y-2">
        <p class="text-xs uppercase tracking-[0.2em] text-primary">Two-factor authentication</p>
        <h1 class="text-2xl sm:text-3xl font-bold tracking-tight leading-tight text-foreground">
          Enter your code to finish signing in.
        </h1>
        <p class="text-muted-foreground text-sm sm:text-base">
          <Show
            when={useRecoveryCode()}
            fallback={<>Open your authenticator app for the 6-digit code.</>}
          >
      ...
      <form onSubmit={handleMfaSubmit} class="space-y-3 sm:space-y-4">
        <Show when={error()}>
        ...
        <div>
          <Label
            for="mfa-code"
            class="block text-xs sm:text-sm font-semibold mb-1 sm:mb-2 text-foreground"
          >
          ...
          <TextFieldRoot>
            <TextField
              ...
              // where most people read their authenticator.
        ...
        <Button
          type="submit"
          disabled={isLoading()}
          class="w-full py-2.5 sm:py-3 rounded-lg font-semibold text-sm sm:text-base"
        >
  ...
  return (
    <Show when={!mfaChallenge()} fallback={<MfaStep />}>
      <AuthLayout>
        <div class="text-left mb-6 space-y-2">
